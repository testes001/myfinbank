...
export function getAllPendingKYCUsers(): Array<{ user: UserModel; kyc: KYCData }> {
  try {
    const kycStored = localStorage.getItem("banking_kyc_data");
    if (!kycStored) return [];

    const allKYCData: Record<string, KYCData> = JSON.parse(kycStored);
    const pendingUsers: Array<{ user: UserModel; kyc: KYCData }> = [];

    // This is a simplified version - in real implementation,
    // we'd need to fetch actual user data from UserORM
    for (const userId in allKYCData) {
      const kyc = allKYCData[userId];
      if (kyc.kycStatus === "pending" || kyc.kycStatus === "under_review") {
        // We'll need to populate actual user data in the component
        pendingUsers.push({
          user: {} as UserModel, // Placeholder
          kyc,
        });
      }
    }
    ...
    console.error("Failed to get pending KYC users:", error);
    return [];
  }
}

// Auto-flag suspicious activity based on transactions
export function checkTransactionForSuspiciousActivity(
  ...
  if (amount > 10000) {
    addSuspiciousActivityFlag({
      ...
      description: `High-value transaction of $${amount.toLocaleString()}`,
    });
  }

  // Additional checks can be added here (rapid transfers, etc.)
}
